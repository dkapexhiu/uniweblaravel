...
class CurriculumDetails extends Model
{
    ...
    public function getAvg($grades, $user, $isNoDropped = false)
    {
        foreach ($grades as $grade) {
            if ($grade->curriculum_details_id == $this->curriculum_details_id) {
                if ( $isNoDropped && ($grade->is_inc && $grade->getAverage() != 0) )
                    return $grade->getAverage();
                else if( $isNoDropped && $grade->getGrade() != 'DRP' && $grade->getAverage() != 0 )
                    return $grade->getAverage();
                else if ( !$isNoDropped )
                    return $grade->getAverage();
            }
        ...
        foreach ($cschools as $cschool) {
            foreach ($cschool->creditedCourses as $ccourse) {
                if($ccourse->curriculum_details_id == $this->curriculum_details_id) {
                    if($isNoDropped && $ccourse->getGrade() != 'DRP' && $ccourse->getAverage() != 0) {
                        if ($ccourse->is_inc && $ccourse->grade != 5)
                            return $ccourse->getAverage();
                        else
                            return $ccourse->getAverage();
                    }
                    else if ( !$isNoDropped )
